/* zip.c
 *
 * This file was generated from zip.xs from Nicholas Clark's
 * ex-lib-zip-0.04, which stated
 *
...
#define CLASSNAME		"ExtUtils::PerlToExe::INC"
...
openzipfile (PerlIO *zipfile, off_t length)
{
  ...
  PerlIO_debug("Hit at %ld\n", (long)searchoffset);
	...
	if ((offset == 0)) {
            PerlIO_debug("Bah, dir at zero.\n");
            return 0;
        }
        if ((size + offset != searchoffset)) {
	  /* Central directory is at start of zip (hence no files in it) or
	     ...
	     actual location.  */
#ifdef DEBUG_LIBZIP
	  PerlIO_debug("Bah. dir not where expected: %ld != %ld.\n",
            (long)(size + offset), (long)(searchoffset));
#endif
	    return 0;
	}
...
findfile (HV *self, struct cache *aswas, PerlIO *fp, SV *file)
{
...

  {
    ...
    layer = PerlIO_find_layer(aTHX_ layer_name, layer_len, 1);
...
XS(XS_ExtUtils_PerlToExe_INC_name)
{
#ifdef dVAR
    dVAR; dXSARGS;
#else
...
#endif
    if (items != 1)
       Perl_croak(aTHX_ "Usage: %s->name", CLASSNAME);
	...
	if (SvROK(ST(0)) && sv_derived_from(ST(0), CLASSNAME)) {
	    self = (LIBZIB_OBJ*)SvRV(ST(0));
	}
	else
	    croak("self is not derived from "CLASSNAME"");

        entry = hv_fetch (self, (char *)self, 0, 0);
        ...
        ST(0) = newSVpvn(name + sizeof(struct cache),
...
XS(XS_ExtUtils_PerlToExe_INC_INC)
{
#ifdef dVAR
    dVAR; dXSARGS;
#else
...
#endif
    if (items != 2)
       Perl_croak(aTHX_ "Usage: %s->INC(%s)", CLASSNAME, "file");
    PERL_UNUSED_VAR(cv); /* -W */
    {
	LIBZIB_OBJ     *self;
	SV             *file = ST(1);
	...
	if (SvROK(ST(0)) && sv_derived_from(ST(0), CLASSNAME)) {
        ...
        zipfile = hv_fetch (self, (char *)self, 0, 0);
...
pl2exe_boot_zip(pTHX)
{
    ...
    newXS(CLASSNAME "::name", XS_ExtUtils_PerlToExe_INC_name, file);
    newXS(CLASSNAME "::INC", XS_ExtUtils_PerlToExe_INC_INC, file);
}
